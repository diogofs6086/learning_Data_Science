...
?train
model_4 <- train(G3 ~ ., data = data_train, method = 'rpart')
varImp(model_4)

# Model 4 - RF model depends on the variables more importants

model_4 <- rpart(G3 ~ G2 + G1 + failures,
                 data = data_train,
                 control = rpart.control(cp = 0.01))
model_4

# Test data set predictions
tree_pred_4 <- predict(model_4, data_test, type = 'class')

# Correct prediction percentage
mean(tree_pred_4 == data_test$G3)

# Confusion Matrix
table(tree_pred_4, data_test$G3)
...
### Conclusion: The Linear Model gives the best results to predict G3.
